...
def cache_checkout_data(request):
    try:
        ...
        stripe.PaymentIntent.modify(pid, metadata={
            'dolly': json.dumps(request.session.get('dolly', {})),
...
def checkout(request):
    ...
    if request.method == 'POST':
        ...
        if order_form.is_valid():
            ...
            order.original_dolly = json.dumps(dolly)
            order.save()
            for item_id, item_data in dolly.items():
                try:
                    furniture = Furniture.objects.get(id=item_id)
...
def checkout_success(request, order_number):
    ...
    if 'dolly' in request.session:
        del request.session['dolly']
